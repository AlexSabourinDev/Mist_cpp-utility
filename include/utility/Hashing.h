...
constexpr uint32_t Hash_djb2(const char* string, const uint32_t hash = BEGIN_HASH_VALUE) {
	return (string[0] == '\0') ? 0 : ((hash << SHIFT_AMOUNT) + hash) + string[1] + Hash_djb2(&string[1], hash);
}
