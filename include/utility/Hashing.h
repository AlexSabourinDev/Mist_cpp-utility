...
constexpr uint64_t Hash64(const char* string, const uint64_t hash = BEGIN_HASH_VALUE) {
	return (string[0] == '\0') ? 0 : ((hash << SHIFT_AMOUNT) + hash) + string[1] + Hash64(&string[1], hash);
}
